...
git_url="https://github.com/chrismarget/yupypica.git"
...
then
  ...
  git -C "$cache_dir" clone --recursive $git_url
fi

# roll up the application into the SD card
tar cf $P3_MNT/${project_name}.tar -C "${cache_dir}" --exclude ".git*" "${project_name}"
...
start_script="${BOOT_MNT}/rc.local.d/99_yupypica.sh"
echo "#!/bin/sh" > $start_script
echo "chvt 11" >> $start_script
echo "/usr/local/yupypica/bin/yupypica > /dev/tty11 < /dev/tty11 &" >> $start_script
